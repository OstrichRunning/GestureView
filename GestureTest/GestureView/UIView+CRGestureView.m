...
- (BOOL)containsPoint:(CGPoint)point {
    CGFloat rotation = [[self.layer valueForKeyPath:@"transform.rotation"] floatValue];
    ...
    /// 真实宽高
    CGFloat realW = sqrt(pow(self.frame.size.width - tmp * tanRotation, 2.0) + pow(self.frame.size.height - tmp, 2.0));
    ...
    if ((fabs(distanceW) > realW / 2.0 || fabs(distanceH) > realH / 2.0)) return NO;
    [self anchorWith:CGPointMake((realW / 2.0 + distanceW) / realW, (realH / 2.0 + distanceH) / realH)];
    return YES;
}
